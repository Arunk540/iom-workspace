...
7. **Per `$repo` in `$repo_order` (topo-sorted):** `workdir = (SINGLE ? $root : <$root>/<$repo>)`, `Bash: cd workdir` · `$workspace_context_dir = <$root>/.contmark` · `$repo_context_dir = <$root>/.contmark/repos/<$repo>` · read ONLY `$matches WHERE repo == $repo` at `source:line` · read `_pins.yml` → `$skills.*` · run Boot → Stage 6.
