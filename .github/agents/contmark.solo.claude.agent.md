...
3. Read `.claude/skills/implementation/contmark-execution-core/SKILL.md` — **once**.
...
**WORKSPACE mode:** capture the created `pr_url` + `commit_sha` into `$previous_repos[]` for the next iteration. PR body must include `Companion PRs:` section listing all sibling-repo PRs from this workspace task. Earlier PRs get the final companion list back-filled via `github/add_issue_comment` after all iterations complete.
...
- **WORKSPACE mode**: ABORT in repo N halts the workspace; previously completed repos keep their PRs (do not revert).
