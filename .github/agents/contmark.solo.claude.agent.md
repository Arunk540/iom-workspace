...
### Stage 3 — Early guard
Read `.claude/skills/contmark-token-usage-prediction/SKILL.md` — §Model cap & budgets + §Calculate only. `liveWindow = pipelineBudget = modelCap` (REAL window — no ×2.5). Compute live-thread tokens: `live% = est ÷ modelCap`. `live% ≥ 85%` → `CONTEXT_PRESSURE` — STOP, compact/split. `est > modelCap` → `RUNAWAY_PIPELINE` — STOP. Do not proceed to Stage 4.
